class QuestManager {
  ...
  bindEvents() {
    ...
    document.getElementById('btn-seed-shadowland-chain')?.addEventListener('click', () => this.seedShadowlandChain());
  ...
  async seedShadowlandChain() {
    const result = await apiClient.seedShadowlandQuestChain();
    const npcSummary = result?.npcs
      ? `NPCs(created=${result.npcs.created}, updated=${result.npcs.updated}, total=${result.npcs.total})`
      : 'NPCs(ok)';
    const questSummary = result?.quests
      ? `Quests(created=${result.quests.created}, updated=${result.quests.updated}, total=${result.quests.total})`
      : 'Quests(ok)';
    alert(`Seed Shadowland concluído. ${npcSummary} ${questSummary}`);
    await this.loadData();
  }
