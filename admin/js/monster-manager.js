class MonsterManager {
  ...
  bindEvents() {
    ...
    document.getElementById('btn-seed-monster-cards')?.addEventListener('click', () => {
      this.seedCards();
    });
    document.getElementById('btn-seed-shadowland-cards')?.addEventListener('click', () => {
      this.seedShadowlandCards();
    });
  ...
  showTemplateForm(template = null) {
    ...
    if (template) {
      ...
      form.elements.difficulty.value = template.difficulty || 'medium';
      form.elements.collection_id.value = template.collection_id || 'monsters_1';
      form.elements.deck_mode.value = template.deck_mode || 'hybrid';
      form.elements.visual.value = template.visual || template.sprite_ref || '';
      const presetSelect = document.getElementById('monster-visual-preset');
      if (presetSelect) {
        const visual = String(form.elements.visual.value || '').toLowerCase();
      ...
      form.elements.manual_deck_cards.value = (template.manual_deck_cards || []).join(',');
      this.currentEditingTemplateId = Number(template.id);
    } else {
      title.textContent = 'Novo Monstro';
      form.elements.collection_id.value = 'monsters_1';
      form.elements.deck_mode.value = 'hybrid';
      const presetSelect = document.getElementById('monster-visual-preset');
      if (presetSelect) {
        presetSelect.value = 'Monstro1';
      }
      form.elements.visual.value = 'Monstro1';
    }
    this.populateDropCardSelect();
  ...
  editTemplate(templateId) {
    const current = this.templates.find((t) => t.id === templateId);
    if (!current) return;
  ...
  async seedCards() {
    if (!confirm('Gerar/atualizar cartas de monstros e deck IA?')) return;
    const result = await apiClient.seedMonsterCards();
    alert(`Cartas processadas: ${result.total} (criadas: ${result.created}, atualizadas: ${result.updated})`);
  }

  async seedShadowlandCards() {
    if (!confirm('Gerar/atualizar Shadowland Creatures (75 cartas) e deck IA?')) return;
    const result = await apiClient.seedShadowlandCards();
    alert(`Shadowland processadas: ${result.total} (criadas: ${result.created}, atualizadas: ${result.updated})`);
    if (window.adminDashboard?.loadCollections) {
      await window.adminDashboard.loadCollections();
    }
    await this.loadData();
  }
