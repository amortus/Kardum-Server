...
// Get full card catalog (all cards + owned flag) — used by the Collection screen
router.get('/catalog', optionalAuth, async (req: AuthRequest, res: Response) => {
  try {
    const cards = await cardRepository.getCatalogCards(req.userId);
    res.json({ cards });
  } catch (error: any) {
    console.error('Get cards catalog error:', error);
    res.status(500).json({ error: 'Failed to get cards catalog' });
  }
});
