...
export class CardRepository {
  private getDefaultGlobalVfxLayout() {
    return {
      ...
      cardbaseImageUrl: null as string | null
    };
  }

  private parseGlobalVfxLayout(row: any) {
  ...
  async getAllCards(filters: {
    ...
    const cards = await dbHelpers.queryAll<any>(
      `SELECT * FROM cards WHERE ${where.join(' AND ')} ORDER BY name ASC`,
      params
    );
    return cards.map(this.parseCard);
  }

  async getCatalogCards(userId?: number): Promise<(Card & { owned: boolean })[]> {
    const cards = await dbHelpers.queryAll<any>(
      `SELECT * FROM cards WHERE is_active = 1 ORDER BY collection_id ASC, name ASC`
    );

    let ownedIds = new Set<string>();
    if (userId && Number(userId) > 0) {
      const unlocked = await this.getUnlockedCardIdsForUser(Number(userId));
      unlocked.forEach((id) => ownedIds.add(String(id)));
    }

    return cards.map((dbCard) => {
      const parsed = this.parseCard(dbCard);
      const owned = parsed.default_unlocked || ownedIds.has(String(dbCard.id));
      return { ...parsed, owned };
    });
  }

  async getCardById(id: string): Promise<Card | null> {
    const card = await dbHelpers.query<any>('SELECT * FROM cards WHERE id = ?', [id]);
    return card ? this.parseCard(card) : null;
  }

  async getUnlockedCardIdsForUser(userId: number): Promise<string[]> {
    const rows = await dbHelpers.queryAll<{ card_id: string }>(
  ...
  async unlockCardForUser(userId: number, cardId: string, source: string = 'monster_drop'): Promise<boolean> {
    const existing = await dbHelpers.query<{ id: number }>(
      'SELECT id FROM user_card_unlocks WHERE user_id = ? AND card_id = ?',
      [userId, cardId]
    );
    if (existing) {
      return false;
    ...
    await dbHelpers.run(
      'INSERT INTO user_card_unlocks (user_id, card_id, source) VALUES (?, ?, ?)',
      [userId, cardId, source]
    );
    return true;
  }

  async getLockedCardsByCollectionForUser(collectionId: string, userId: number): Promise<Card[]> {
  ...
  async getCardsByType(type: string): Promise<Card[]> {
    const cards = await dbHelpers.queryAll<any>(
      'SELECT * FROM cards WHERE type = ? AND is_active = 1',
      [type]
    );
    return cards.map(this.parseCard);
  }

  async getCardsByRace(race: string): Promise<Card[]> {
    const cards = await dbHelpers.queryAll<any>(
      'SELECT * FROM cards WHERE race = ? AND is_active = 1',
      [race]
    );
    return cards.map(this.parseCard);
  }

  async createCard(card: Omit<Card, 'created_at' | 'updated_at'>): Promise<void> {
    await dbHelpers.run(
      `INSERT INTO cards (id, name, type, race, class, cost, attack, defense, abilities, text, rarity, image_url, card_image_url, effects, hero_power_text, hero_power_cost, hero_power_effect, passive_effect, default_unlocked, visual_auras, collection_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        card.id,
        card.name,
    ...
    await dbHelpers.run(
      `UPDATE cards 
       SET name = ?, type = ?, race = ?, class = ?, 
           ...
           image_url = ?, card_image_url = ?, effects = ?, 
  ...
  private serializeEffects(card: Partial<Card>): string | null {
  ...
  async deleteCard(id: string): Promise<void> {
    await dbHelpers.run('UPDATE cards SET is_active = 0 WHERE id = ?', [id]);
  }

  async getCardLayout(cardId: string): Promise<any | null> {
    const layout = await dbHelpers.query<any>(
      'SELECT * FROM card_layouts WHERE card_id = ?',
      [cardId]
    );
    return layout || null;
  }

  async getAllCardArtworkLayouts(): Promise<Array<{
    cardId: string;
    artwork: {
    ...
    const rows = await dbHelpers.queryAll<any>(
      `SELECT card_id, artwork_offset_left, artwork_offset_top, artwork_offset_right, artwork_offset_bottom,
  ...
  async getGlobalVfxLayout(): Promise<any> {
    const row = await dbHelpers.query<any>(
      'SELECT * FROM card_vfx_layout_global WHERE id = ?',
      [1]
    );
    return this.parseGlobalVfxLayout(row);
  }

  async saveGlobalVfxLayout(layout: any): Promise<void> {
  ...
  async saveCardLayout(cardId: string, layout: any): Promise<void> {
  ...
  async syncAllCards(): Promise<{ synced: number; errors: string[] }> {
    const allCards = await dbHelpers.queryAll<any>('SELECT * FROM cards');
    let synced = 0;
    const errors: string[] = [];
    ...
    for (const dbCard of allCards) {
      try {
        ...
        await dbHelpers.run(
          'UPDATE cards SET effects = ? WHERE id = ?',
          [effectsJson, card.id]
        );
        synced++;
      } catch (err: any) {
  ...
  async getCardsByDeck(deckId: number): Promise<Card[]> {
    const deck = await dbHelpers.query<any>('SELECT cards FROM decks WHERE id = ?', [deckId]);
    if (!deck || !deck.cards) {
      return [];
    }

    const cardIds: string[] = JSON.parse(deck.cards);
    if (cardIds.length === 0) {
      return [];
    }
    ...
    const cards = await dbHelpers.queryAll<any>(
      `SELECT * FROM cards WHERE id IN (${placeholders}) AND is_active = 1`,
      cardIds
    );
  ...
  private parseCard(dbCard: any): Card {
    ...
    return {
      ...
      card_image_url: dbCard.card_image_url ?? undefined,
