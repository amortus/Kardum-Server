...
export class CardRepository {
  ...
  async createCard(card: Omit<Card, 'created_at' | 'updated_at'>): Promise<void> {
    await dbHelpers.run(
      `INSERT INTO cards (id, name, type, race, class, cost, attack, defense, abilities, text, rarity, image_url, effects, hero_power_text, hero_power_cost, hero_power_effect, passive_effect, default_unlocked, visual_auras, collection_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        card.id,
        card.name,
