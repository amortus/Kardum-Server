...
class RankService {
  ...
  buildProfile(user: User): PlayerProfile {
    ...
    const wz = String(user.world_zone || '').trim();
    const wx = user.world_x != null ? Number(user.world_x) : NaN;
    const wy = user.world_y != null ? Number(user.world_y) : NaN;
    const world_position =
      wz !== '' && Number.isFinite(wx) && Number.isFinite(wy)
        ? { zone: wz, x: wx, y: wy }
        : null;
    const gold = user.gold != null && Number.isFinite(Number(user.gold)) ? Number(user.gold) : 0;
