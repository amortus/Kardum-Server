...
export class UserRepository {
  ...
  async getWorldPosition(userId: number): Promise<{
    zone: string;
    x: number;
    y: number;
    updatedAt: number;
    ...
    const row = await dbHelpers.query<any>(
      'SELECT world_zone, world_x, world_y, world_updated_at FROM users WHERE id = ?',
      [userId]
    );
    if (!row) return null;
    const zone = String(row.world_zone || '').trim();
    const x = Number(row.world_x);
    const y = Number(row.world_y);
    const updated = row.world_updated_at;
    if (!zone || !Number.isFinite(x) || !Number.isFinite(y)) return null;
    const updatedAtMs = updated ? Date.parse(String(updated)) : NaN;
    return {
      zone,
      x,
      y,
  ...
  async updateWorldPosition(userId: number, zone: string, x: number, y: number): Promise<void> {
    await dbHelpers.run(
      `UPDATE users
       SET world_zone = ?, world_x = ?, world_y = ?, world_updated_at = CURRENT_TIMESTAMP
       WHERE id = ?`,
      [zone, Math.round(x), Math.round(y), userId]
    );
  }
