...
export class MonsterRepository {
  ...
  async clearTemplateDrops(templateId: number): Promise<void> {
    await dbHelpers.run(
      `UPDATE monster_template_drops
       SET is_active = 0, updated_at = CURRENT_TIMESTAMP
       WHERE template_id = ?`,
  ...
  async removeSpawn(spawnUid: string): Promise<number> {
    const result = await dbHelpers.run(
      'UPDATE monster_spawns SET is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE spawn_uid = ? AND is_active = 1',
      [spawnUid]
    );
