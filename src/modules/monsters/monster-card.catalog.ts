...
const defenders = Object.entries(monsterNamesByRace).flatMap(([raceKey, entries]) =>
  entries.map((entry, idx) => {
    const cost = 2 + (idx % 6);
    const attack = Math.max(1, cost + (idx % 2));
    const defense = Math.max(2, cost + 2 + (idx % 3));
    const race = raceKey as keyof typeof Race;
    const idSeed = defenderIdSeeds[raceKey as keyof typeof defenderIdSeeds][idx];
    return {
      id: `m1_mon_${slug(idSeed)}`,
      name: entry.name,
      type: CardType.DEFENDER,
      race: Race[race.toUpperCase() as keyof typeof Race],
      cost,
      attack,
      defense,
...
const abilityCards = spells.map((spell, idx) => {
  ...
  const type = isHeal ? CardType.CONSUMABLE : CardType.ABILITY;
  const cost = 1 + (idx % 5);
  return {
...
export const MONSTER_CARD_CATALOG = [
  ...defenders,
  ...abilityCards,
  ...equipmentCards,
