...
function rarityByCost(cost: number): Rarity {
  if (cost >= 7) return Rarity.LEGENDARY;
  if (cost >= 5) return Rarity.EPIC;
  if (cost >= 3) return Rarity.RARE;
...
const raceCycle = [Race.ORC, Race.DEVA, Race.ELF, Race.DWARF, Race.HUMAN];
...
const shadowlandSpells = spellDefinitions.map((spell, idx) => {
  ...
  return {
    ...
    name: spell.name,
    type: isConsumable ? CardType.CONSUMABLE : CardType.ABILITY,
    race: null,
    cost,
...
function monsterImageByName(name: string): string {
...
export const SHADOWLAND_CARD_CATALOG = [
  ...defenders,
  ...shadowlandSpells,
...
if (SHADOWLAND_CARD_CATALOG.length !== 95) {
  throw new Error(`Shadowland catalog must contain 95 cards, got ${SHADOWLAND_CARD_CATALOG.length}`);
}

export const SHADOWLAND_COLLECTION = {
