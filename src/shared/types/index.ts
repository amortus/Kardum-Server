...
export interface Card {
  ...
  rarity: Rarity;
  image_url?: string;
  /** Imagem completa da carta (full-bleed); HUD é desenhada em runtime no cliente. */
...
export interface CardInstance extends Card {
  ...
  hasAttacked: boolean;
  isSummoned: boolean;
  /** Índice fixo 0..MAX_FIELD-1 no tabuleiro (não compacta ao morrer — evita confusão de alvo). */
  fieldSlot?: number;
...
export interface PlayerProfile {
  ...
  created_at: Date;
  gold: number;
  /** Última posição no mundo (DB); usada pelo cliente para spawn no Shadowland. */
  world_position: { zone: string; x: number; y: number } | null;
}
...
export interface User {
  ...
  world_x?: number | null;
  world_y?: number | null;
  world_updated_at?: Date | string | null;
  gold?: number | null;
  created_at: Date;
  last_login?: Date;
  is_admin: boolean;
