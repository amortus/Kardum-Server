...
async function importCardsFromJS() {
  try {
    ...
    const jsFilePath = path.join(__dirname, '../../client/js/data/cards-database.js');
    ...
    for (const card of cards) {
      try {
        ...
        if (existingCardIds.has(card.id)) {
          ...
          await cardRepository.updateCard(card.id, cardData);
          console.log(`  🔄 Updated: ${card.name} (${card.id})`);
          updated++;
        } else {
          // Insert new card
          await cardRepository.createCard(cardData as any);
