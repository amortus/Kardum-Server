...
function parseAbilities(value: string): string[] {
  if (!value || !value.trim()) return [];
  const v = value.trim();
  if (v.startsWith('[')) {
    try {
      const arr = JSON.parse(v);
      return Array.isArray(arr) ? arr.map((a: unknown) => String(a)) : [];
    } catch {
      return [];
    }
  }
...
async function processOneCSV(
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const get = (col: number) => (col >= 0 && col < values.length ? values[col].trim() : '');
    ...
    if (!id || !name || !type) {
      console.warn(`  ⏭️ ${path.basename(absolutePath)} linha ${i + 1}: ignorada (id/name/type vazios).`);
      stats.skipped++;
      continue;
    }
    ...
    const cardData = {
      ...
      effect: undefined as Card['effect']
    } as Omit<Card, 'created_at' | 'updated_at'>;

    if (heroPowerTextCol >= 0) cardData.hero_power_text = get(heroPowerTextCol) || undefined;
...
async function importCardsFromCSV() {
  ...
  const absolutePaths = csvPaths.map((p) => (path.isAbsolute(p) ? p : path.join(cwd, p)));
